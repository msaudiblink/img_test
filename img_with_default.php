<?php
/**
 * Enhanced Image Retrieval Endpoint with Tag Support and Default Image
 * 
 ...
 * Returns a default image when requested image is not found.
 */
...
// If no image found, use default image if it exists, otherwise use placeholder
if (!$imagePath || !file_exists($imagePath)) {
    // Check for explicit placeholder request
    ...
    // Use default image if it exists
    if (file_exists($defaultImagePath)) {
        $imagePath = $defaultImagePath;
        $usingDefaultImage = true;
        error_log("Using default image for document ID: $id");
    } else {
        // If default image doesn't exist, create informative JSON response
        header('Content-Type: application/json');
        http_response_code(404);
        echo json_encode([
            'status' => 'error',
            'message' => 'Image not found for document ID: ' . $id . ' and default image does not exist',
